=pod
 - data analysis
 Summarizing text documents and spreadsheet statistics
 - Gmail 資料保護深入分析
 此功能延伸自 Google 雲端硬碟的深入分析功能，可協助您更全面掌握機構的資料安全。Gmail 資料保護深入分析會利用 Google Workspace 現行資料遺失防護 (DLP) 規則的中繼資料，整理出概括性的整體統計資料，協助您：a) 掌握內部人員透過 Gmail 分享機密資料的趨勢 b) 找出機構中潛藏的風險區域 c) 評估現行資料遺失防護政策的成效
